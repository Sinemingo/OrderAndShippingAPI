...
namespace OrderAndShippingAPI.Data.Concrete.EFCore.Contexts
{
    public class OrderAndShippingContext:DbContext
    {
        ...
        public static DbContextOptions<OrderAndShippingContext> _options;
        public OrderAndShippingContext(DbContextOptions<OrderAndShippingContext> options) : base(options)
        {
            _options = options;
            foreach (var item in _options.Extensions)
            {
        ...
        public static void MigrationUpdate()
        {
            try
            {
                using (var db = new OrderAndShippingContext(_options))
                {
                    //db.Orders.Include(o => o.OrderLines).ThenInclude(x => x.Product);
                    db.Database.Migrate();
                }
            }
            catch (Exception err)

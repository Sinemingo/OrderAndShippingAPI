...
namespace OrderAndShippingAPI.Services.Concrete
{
    public class ProductManager : IProductService
    {
        ...
        public async Task<IDataResult<Product>> Get(int productId)
        {
            ...
            if (product != null)
            {
                return new DataResult<Product>(Entities.Abstract.Enums.EResultStatu.Success,product);
            }
            return new DataResult<Product>(Entities.Abstract.Enums.EResultStatu.Error,null, "Ürün Bulunamadı!");
        }
        ...
        public async Task<IDataResult<IList<Product>>> GetAll()
        {
            ...
            if (products.Count>-1)
            {
                return new DataResult<IList<Product>>(Entities.Abstract.Enums.EResultStatu.Success,products);
            }
            return new DataResult<IList<Product>>(Entities.Abstract.Enums.EResultStatu.Error, null, "Ürünler Bulunamadı!");
        }

        public async Task<IDataResult<IList<Product>>> GetAllByCategory(int categoryId)
        {
            ...
            return new DataResult<IList<Product>>(Entities.Abstract.Enums.EResultStatu.Error, null, "Böyle Bir Kategori Bulunamadı!");
        }

        public Task<IResult> Update(Product productUpdateDto)
        {

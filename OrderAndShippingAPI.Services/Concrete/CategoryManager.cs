...
namespace OrderAndShippingAPI.Services.Concrete
{
    public class CategoryManager : ICategoryService
    {
        ...
        public async Task<IDataResult<CategoryDto>> Get(int categoryId)
        {
            ...
            if (category != null)
            {
                return new DataResult<CategoryDto>(Entities.Abstract.Enums.EResultStatu.Success, new CategoryDto
                {
                    Category=category,
                    ResultStatus=Entities.Abstract.Enums.EResultStatu.Success
                });
            }
            return new DataResult<CategoryDto>(Entities.Abstract.Enums.EResultStatu.Error, null, "Kategori Bulunamadı!");
        }
        ...
        {
            var category = await _unitOfWork.Categories.GetAsync(c => c.CategoryId == categoryUpdateDto.CategoryId);
            if (category != null)
            {
                category = _mapper.Map<Category>(categoryUpdateDto);
                await _unitOfWork.Categories.UpdateAsync(category);
                await _unitOfWork.SaveAsync();
                return new Result(Entities.Abstract.Enums.EResultStatu.Success, $"{categoryUpdateDto.CategoryName} adlı kategori güncellenmiştir.");
            }
            return new Result(Entities.Abstract.Enums.EResultStatu.Error, $"{categoryUpdateDto.CategoryName} adlı kategori bulanamamıştır.");
        }
    }
}
